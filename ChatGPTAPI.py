...
def main():
    ...
    with st.expander("음성 비서 프로그램에 관하여", expanded = True):
        st.write(
            ...
            - TTS(Text to Speech)는 구글의 Google Translate TTS를 활용햇습니다.
            """
        )
...
def ask_gpt(prompt,model):
    ...
    with col2:
        # 오른쪽 영역 작성
        st.subheader("질문/답변")
        if (audio.duration.seconds > 0) and (st.session_state["check_reset"] == False):
            # ChatGPT에게 답변 얻기
            response = ask_get(st.session_state["messages"], model)
            ...
            # 채팅 시각화를 위한 답변 내용 저장
            now = datetime.now().strftime("%H:%M")
            st.seesion_state["chat"] = st.session_state["chat"] + [("bot", now, response)]
